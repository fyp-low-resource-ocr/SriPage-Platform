...
export class JobsController {
  ...
  methods() { return this.parsers.list(); }

  @Post('uploads/presign')
  @ApiOperation({ summary: 'Create a presigned upload URL' })
  @ApiResponse({ status: 201, schema: { example: { objectKey: 'inputs/uuid-invoice.pdf', uploadUrl: 'http://localhost:9000/...' } } })
  presign(@Body() dto: PresignUploadDto) { return this.jobs.presign(dto); }
  ...
  @ApiParam({ name: 'id', format: 'uuid' })
  @ApiResponse({ status: 200, schema: { example: { url: 'http://localhost:9000/...' } } })
  @ApiResponse({ status: 404, description: 'Job not found or result is not ready.' })
  result(@Param('id') id: string) { return this.jobs.getResultUrl(id); }
}
